...
const PRODUCT_NAME_SIGNAL = /\b(?:saw|drill|grinder|fan|hammer|wrench|driver|sander|cutter|compressor|vacuum|generator|welder|sprayer|level|light|blower|trimmer|mower|pump|charger|battery|tool\s*bag|set|kit)\b/i;
const SPEC_ONLY_LINE = /^(?:(?:wood|metal|steel|concrete|masonry|aluminium|cutting)\s*(?:capacity)?\s*:|(?:bl|brushless)\s+motor\b)/i;
...
function findTitle(lines: string[], model: string) {
  const candidates = lines.filter((line) => {
    ...
    if (SPEC_ONLY_LINE.test(line)) return false;
    const letters = (line.match(/[A-Za-z]/g) || []).length;
    return letters >= 6;
  });
  return candidates.find((line) => PRODUCT_NAME_SIGNAL.test(line) && /[A-Z]{3}/.test(line))
    || candidates.find((line) => !SPEC_SIGNAL.test(line) && /[A-Z]{3}/.test(line))
    || candidates[0]
    || "";
}
