...
export class LiveBufferService implements BufferService {
  ...
  async createPost(input:BufferCreateInput){const due=input.dueAt?` dueAt:${gqlString(input.dueAt)}`:"";const data=await this.client.request<{createPost:{post?:BufferPostResult;message?:string}}>(`mutation { createPost(input:{text:${gqlString(input.text)} channelId:${gqlString(input.channelId)} schedulingType:automatic mode:${input.mode}${due} assets:[{image:{url:${gqlString(input.imageUrl)}}}]}) { ... on PostActionSuccess { post { id status dueAt } } ... on MutationError { message } } }`);if(!data.createPost.post)throw new BufferError(data.createPost.message||"Buffer rejected the post","MUTATION_ERROR",data.createPost);return data.createPost.post;}
