...
import { getFinalArtwork } from "./final-artwork-service";
...
export async function processSocialPost(socialPostId:string, action:"SOCIAL_POST_SCHEDULE"|"SOCIAL_POST_PUBLISH_NOW"|"SOCIAL_POST_RETRY", user:CurrentUser, simulateInstagramFailure=false,platformCaptions?:Record<string,string>) {
  authorize(action==="SOCIAL_POST_SCHEDULE"?"MARKETING":"REVIEW",user);
  ...
  const advert=await prisma.advertisement.findUniqueOrThrow({where:{id:post.advertisementId}});
  if(advert.status!=="APPROVED") throw new Error("ADVERT_NOT_APPROVED");
  const artwork=await getFinalArtwork(advert);
  if(post.finalArtworkData!==artwork.path || post.finalArtworkUrl==="pending") throw new Error("FINAL_ARTWORK_MISSING");
...
export async function createSocialPost(input:{idempotencyKey:string;advertisementId:string;caption:string;platformCaptions?:Record<string,string>;channelIds:string[];mode:"SCHEDULE"|"NOW";scheduledLocal?:string;timezone:string;artworkDataUrl?:string;simulateInstagramFailure?:boolean}, user:CurrentUser) {
  authorize(input.mode==="NOW"?"REVIEW":"MARKETING",user);
  const advert=await prisma.advertisement.findUnique({where:{id:input.advertisementId}});
  if(!advert)throw new Error("NOT_FOUND");if(advert.status!=="APPROVED")throw new Error("ADVERT_NOT_APPROVED");
  const artwork=await getFinalArtwork(advert);
  if(input.artworkDataUrl && input.artworkDataUrl!==artwork.path)throw new Error("ARTWORK_REQUIRES_REVIEW");
  const existing=await prisma.socialPost.findUnique({where:{idempotencyKey:input.idempotencyKey},include:{channels:{include:{socialChannel:true}},attempts:true}});
  if(existing){if(existing.advertisementId!==advert.id||existing.finalArtworkData!==artwork.path)throw new Error("FORBIDDEN");return existing;}
  ...
  const buffer=getBufferService();
  const post=await prisma.socialPost.create({data:{idempotencyKey:input.idempotencyKey,advertisementId:advert.id,caption:input.caption,status:"PUBLISHING",mode:input.mode,scheduledAt,timezone:input.timezone,finalArtworkUrl:"pending",finalArtworkData:artwork.path,dryRun:buffer.dryRun,createdByUserId:user.id,channels:{create:channels.map(channel=>({socialChannelId:channel.id,status:"READY",intendedSchedule:scheduledAt}))}}});
  const stored=await getMediaStorage().upload(`adverts/${advert.id}/social-posts/${post.id}.png`,artwork.path), finalArtworkUrl=stored.provider==="DATABASE"?`/api/social-posts/${post.id}/artwork`:stored.url;
  await prisma.socialPost.update({where:{id:post.id},data:{finalArtworkUrl,finalArtworkData:artwork.path}});
