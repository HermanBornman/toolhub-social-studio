...
return detectForegroundCrop(pixels, width, height);
}

export async function cropPreview(pageDataUrl: string, crop: ImageCrop) {
const source = await loadImage(pageDataUrl);
const safeCrop = normaliseCrop(crop);
const sourceX = Math.round(source.width * safeCrop.x);
const sourceY = Math.round(source.height * safeCrop.y);
const sourceWidth = Math.max(1, Math.min(source.width - sourceX, Math.round(source.width * safeCrop.width)));
const sourceHeight = Math.max(1, Math.min(source.height - sourceY, Math.round(source.height * safeCrop.height)));
const canvas = document.createElement("canvas");
canvas.width = sourceWidth;
canvas.height = sourceHeight;
const context = canvas.getContext("2d");
if (!context) throw new Error("Product preview canvas unavailable");
...
if (retainLargestSubject) {
retainLargestAlphaComponent(pixels.data, canvas.width, canvas.height);
}
