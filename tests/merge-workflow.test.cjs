...
require.cache[authPath]={id:authPath,filename:authPath,loaded:true,exports:{
 getCurrentUser:async()=>{const role=process.env.TOOLHUB_USER_ROLE;if(!['STORE_MANAGER','STAFF','MARKETING','MANAGER','ADMIN'].includes(role))throw Error('UNAUTHENTICATED');const id=process.env.TOOLHUB_USER_ID;return{id,name:'Toolhub '+role,email:id+'@toolhub.local',role,branchId:process.env.TOOLHUB_USER_BRANCH_ID||null}},
 withRequestUser:(_user,fn)=>fn()
}};
const route=name=>require('../.test-dist/app/api/'+name+'/route');
const actor=(role,id=role.toLowerCase(),branchId=null)=>{process.env.TOOLHUB_USER_ROLE=role;process.env.TOOLHUB_USER_ID=id;if(branchId)process.env.TOOLHUB_USER_BRANCH_ID=branchId;else delete process.env.TOOLHUB_USER_BRANCH_ID;return {id,name:'Toolhub '+role,email:id+'@toolhub.local',role,branchId}};
...
let staff,manager,admin,storeManager;
test.before(async()=>{
 ...
 staff=actor('STAFF','staff-test');manager=actor('MANAGER','manager-test');admin=actor('ADMIN','admin-test');storeManager=actor('STORE_MANAGER','store-manager-test');
 for(const user of [staff,manager,admin])await prisma.user.create({data:user});
 await prisma.user.create({data:storeManager});
});
...
test('Store Manager submit endpoint is 403 and cannot transition DRAFT to AWAITING_APPROVAL',async()=>{actor('STORE_MANAGER',storeManager.id);const response=await route('adverts/[id]/submit').POST(req({}),params('missing'));assert.equal(response.status,403);const created=await createAdvert();actor('STORE_MANAGER',storeManager.id);const denied=await route('adverts/[id]/submit').POST(req({}),params(created.id));assert.equal(denied.status,403);assert.equal((await prisma.advertisement.findUniqueOrThrow({where:{id:created.id}})).status,'DRAFT')});
test('Admin submission behavior remains available and audited for preserved workflow',async()=>{const created=await createAdvert();actor('ADMIN',admin.id);const response=await route('adverts/[id]/submit').POST(req({expectedUpdatedAt:created.updatedAt,artworkDataUrl:dataUrl(1080,1350,false)}),params(created.id));assert.equal(response.status,200,await response.clone().text());const advert=await prisma.advertisement.findUniqueOrThrow({where:{id:created.id}});assert.equal(advert.status,'AWAITING_APPROVAL');assert.equal(advert.submittedByUserId,admin.id)});
test('unassigned Store Manager cannot create or finalize a branch-scoped advert',async()=>{actor('STORE_MANAGER',storeManager.id);let response=await route('adverts').POST(req(input()));assert.equal(response.status,409);assert.match((await response.json()).error,/Admin branch assignment required/);const created=await createAdvert();actor('STORE_MANAGER',storeManager.id);response=await route('adverts/[id]/finalize').POST(req({artworkDataUrl:dataUrl(1080,1350,false)}),params(created.id));assert.equal(response.status,409);assert.equal((await prisma.advertisement.findUniqueOrThrow({where:{id:created.id}})).status,'DRAFT')});
test('explicit Admin branch assignment enables Store Manager create and finalize',async()=>{const branch=await prisma.branch.findUniqueOrThrow({where:{id:'branch-stonewood'}});actor('ADMIN',admin.id);let response=await route('admin/users').PATCH(req({id:storeManager.id,branchId:branch.id},'PATCH'));assert.equal(response.status,200,await response.clone().text());actor('STORE_MANAGER',storeManager.id,branch.id);response=await route('adverts').POST(req({...input(),branchId:'branch-polokwane-crossing'}));assert.equal(response.status,403);response=await route('adverts').POST(req({...input(),branchId:branch.id}));assert.equal(response.status,201,await response.clone().text());const created=await response.json();response=await route('adverts/[id]/finalize').POST(req({artworkDataUrl:dataUrl(1080,1350,false)}),params(created.id));assert.equal(response.status,200,await response.clone().text());const advert=await prisma.advertisement.findUniqueOrThrow({where:{id:created.id}});assert.equal(advert.status,'FINALIZED');assert.equal(advert.branchId,branch.id);assert.equal(advert.finalArtworkWidth,1080);assert.equal(advert.finalArtworkHeight,1350)});
test('server rejects every non-DRAFT finalization transition',async()=>{for(const status of ['CHANGES_REQUESTED','AWAITING_APPROVAL','APPROVED','FINALIZED','REJECTED']){const created=await createAdvert();await prisma.advertisement.update({where:{id:created.id},data:{status}});actor('ADMIN',admin.id);const response=await route('adverts/[id]/finalize').POST(req({artworkDataUrl:dataUrl(1080,1350,false)}),params(created.id));assert.equal(response.status,403,status);assert.equal((await prisma.advertisement.findUniqueOrThrow({where:{id:created.id}})).status,status)}});
