...
test("keeps technical capacities out of the product-name field", () => {
  const result = analyseProductText("", "INGCO\nP20S\nBL MOTOR\nWood: 210mm\nMetal: 12mm\nBattery and charger sold separately");
  assert.equal(result.title.value, "Not found");
  assert.deepEqual(result.specs.map((item) => item.value), ["Wood: 210 mm", "Metal: 12 mm"]);
  assert.match(result.excluded[0].value, /sold separately/i);
});
