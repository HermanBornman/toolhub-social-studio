...
test("future statuses remain reserved and required audit actions exist",()=>{assert.deepEqual(ADVERT_STATUSES.slice(0,5),["DRAFT","AWAITING_APPROVAL","CHANGES_REQUESTED","APPROVED","REJECTED"]);for(const action of ["SUBMIT_FOR_APPROVAL","REQUEST_CHANGES","APPROVE","REJECT","PRODUCT_CREATE","PRODUCT_UPDATE","PDF_IMPORT","PDF_PAGE_ANALYSIS","PDF_REVIEW_CORRECTION","PDF_REVIEW_APPROVE","PDF_DRAFT_CREATE"])assert.ok(AUDIT_ACTIONS.includes(action))});
