...
export default async function ApprovalsPage({searchParams}:{searchParams:Promise<{status?:string}>}) { await requirePageUser();  const user=(await requirePageUser());if(!["MARKETING","MANAGER","ADMIN"].includes(user.role))redirect("/"); const status=(await searchParams).status||"AWAITING_APPROVAL"; const adverts=await prisma.advertisement.findMany({where:status==="ALL"?undefined:{status},include:{createdBy:{select:{name:true}},submittedBy:{select:{name:true}}},orderBy:{submittedAt:"desc"}}); return <AppShell title="Approvals" subtitle="Review, comment, and protect every advert before release.">
