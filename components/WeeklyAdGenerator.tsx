...
import { saveAuditRecord } from "@/lib/audit-storage";
import { findCatalogProduct } from "@/lib/catalog";
import { analyseProductText, hasLowConfidence } from "@/lib/document-analysis";
import { createPageCropCandidate, isolateUploadedPhoto, removeCandidateBackground } from "@/lib/image-processing";
...
function known(value: string) {
  return value !== "Not found" && value !== "Unclear—review required" && value !== "Confirmation required" ? value : "";
}
...
export default function WeeklyAdGenerator() {
  ...
  const updateDraft = (id: string, updater: (draft: PageDraft) => PageDraft) => setDrafts((items) => items.map((item) => item.id === id ? updater(item) : item));
  ...
  const selectCandidate = async (page: SupplierPage, analysis: ProductAnalysis) => {
    ...
    const fallback = await createPageCropCandidate(page.dataUrl, page.page, catalog?.imageCrop, catalog ? "medium" : "low");
  ...
  const analysePages = async (pages: SupplierPage[], filename: string) => {
    ...
    try {
      ...
      for (let index = 0; index < pages.length; index++) {
        ...
        setStatus(`Page ${page.page}: selecting the highest-quality product image...`);
        const { candidates, selected } = await selectCandidate(page, analysis);
        setStatus(`Page ${page.page}: removing the selected image background...`);
        let cleaned = "";
        let imageError = "";
        try {
          cleaned = await removeCandidateBackground(selected, known(analysis.model.value) || `page-${page.page}`, (amount) => {
            setProgress(Math.round(((index + 0.55 + amount * 0.4) / pages.length) * 100));
          });
        } catch {
          imageError = "Background removal failed—replace or reprocess the product image before approval.";
        }
        const nettPrice = analysis.nettPrice.value;
        const draft: PageDraft = {
          ...
          imageConfidence: selected.confidence,
          sellingPrice: nettPrice ? calculateSellingPrice(nettPrice, Number(markup) || 55.8) : null,
          sellingPriceOverridden: false,
          markup: Number(markup) || 55.8,
          userCorrections: [],
  ...
  const editNettPrice = (draft: PageDraft, value: string) => {
    ...
    updateDraft(draft.id, (current) => ({
      ...
      analysis: {
        ...current.analysis,
        nettPrice: { value: Number.isFinite(numeric) && numeric > 0 ? numeric : null, confidence: numeric > 0 ? "high" : "low", source: "user" },
      },
      sellingPrice: numeric > 0 ? calculateSellingPrice(numeric, current.markup) : null,
      sellingPriceOverridden: false,
  ...
  const editPromotionalPrice = (draft: PageDraft, value: string) => {
    ...
    updateDraft(draft.id, (current) => ({
      ...
      analysis: {
        ...current.analysis,
        promotionalPrice: { value: Number.isFinite(numeric) && numeric > 0 ? numeric : null, confidence: numeric > 0 ? "high" : "low", source: "user" },
      },
  ...
  const approveAndGenerate = async (draft: PageDraft) => {
    ...
    if (!title || !draft.cleanedProductImage || draft.sellingPrice === null) {
    ...
    if ((hasLowConfidence(draft.analysis) || draft.imageConfidence === "low" || draft.analysis.warnings.length > 0) && !draft.acknowledgedWarnings) {
    ...
    try {
      ...
      const audit: AdvertAuditRecord = {
        ...
        pricing: {
          ...
          markup: draft.markup,
          calculatedSellingPrice: draft.analysis.nettPrice.value ? calculateSellingPrice(draft.analysis.nettPrice.value, draft.markup) : null,
  ...
  const downloadZip = async () => {
    const zip = new JSZip();
    batch.forEach((item) => zip.file(`toolhub-page-${String(item.page).padStart(2, "0")}-${slug(item.product.model || item.product.title)}.png`, item.advert.split(",")[1], { base64: true }));
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        <aside className="editor-panel">
          ...
          <section className="form-section">
            ...
            <Field label="Markup added (%)"><input inputMode="decimal" value={markup} onChange={(event) => {
              ...
              if (Number.isFinite(percentage)) setDrafts((items) => items.map((draft) => ({
                ...draft,
                markup: percentage,
                sellingPrice: draft.sellingPriceOverridden || !draft.analysis.nettPrice.value
                  ? draft.sellingPrice
          ...
          <section className="form-section supplier-section">
            ...
            <p className="section-help">Every page is analysed completely before its product image is processed. An advert is created only after approval.</p>
            <button className="button primary full" type="button" disabled={processing} onClick={() => supplierInputRef.current?.click()}>▤ Choose PDF or image</button>
          ...
          {activeDraft ? <section className="form-section review-section">
            ...
            <Field label="Selected product image"><select value={activeDraft.selectedImageId} disabled={processing} onChange={(event) => chooseImage(activeDraft, event.target.value)}>{activeDraft.imageCandidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.label} · {candidate.confidence}</option>)}</select></Field>
            <button className="button secondary full" type="button" disabled={processing} onClick={() => replacementInputRef.current?.click()}>Replace selected product image</button>
            ...
            {activeDraft.analysis.specs.length ? activeDraft.analysis.specs.map((spec, index) => <div className="review-row" key={`${index}-${spec.value}`}><ConfidenceBadge value={spec} /><input value={spec.value} onChange={(event) => editSpec(activeDraft, index, event.target.value)} /><button type="button" aria-label={`Remove specification ${index + 1}`} onClick={() => removeSpec(activeDraft, index)}>×</button></div>) : <div className="status error">No specifications found—add confirmed information manually.</div>}

            <div className="two-col">
              <Field label="Confirmed nett price"><input inputMode="decimal" value={activeDraft.analysis.nettPrice.value ?? ""} placeholder="Confirmation required" onChange={(event) => editNettPrice(activeDraft, event.target.value)} /></Field>
              ...
              <Field label="Final selling price"><input inputMode="numeric" value={activeDraft.sellingPrice ?? ""} placeholder="Not calculated" onChange={(event) => {
                ...
                updateDraft(activeDraft.id, (current) => ({ ...current, sellingPrice: value > 0 ? Math.round(value) : null, sellingPriceOverridden: true, userCorrections: [...current.userCorrections, "selling price overridden"] }));
              }} /></Field>
            </div>
            <p className="calculation-note">Selling price = nett × {(1 + (Number(markup) || 55.8) / 100).toFixed(3)}, rounded to the nearest rand. No calculation occurs without a confirmed nett price.</p>
        ...
        <section className="preview-panel">
          ...
          {batch.length ? <div className="batch-results"><div className="batch-title"><div><p className="eyebrow">Approved batch</p><strong>{batch.length} advert{batch.length === 1 ? "" : "s"}</strong></div><button className="button primary" type="button" onClick={downloadZip}>Download approved ZIP</button></div><div className="batch-grid">{batch.map((item) => <article key={`${item.page}-${item.product.model}`}><img src={item.advert} alt={`Approved advert for ${item.product.title}`} /><small>PAGE {item.page} · AUDITED</small><strong>{item.product.title}</strong><span>{formatPrice(item.sellingPrice)}</span><button type="button" onClick={() => downloadDataUrl(item.advert, `toolhub-${slug(item.product.model || item.product.title)}.png`)}>Download</button></article>)}</div></div> : null}
        </section>
      </div>
    </main>
  );
