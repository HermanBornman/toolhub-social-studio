...
type User = { id: string; email: string; name: string; role: string; active: boolean; branchId: string | null; branch: Branch | null };

export function AdminUsers({ initialUsers, branches }: { initialUsers: User[]; branches: Branch[] }) {
  const router = useRouter();
  const [notice, setNotice] = useState("");
  const [busy, setBusy] = useState(false);
  const [form, setForm] = useState({ email: "", name: "", role: "STORE_MANAGER", branchId: branches[0]?.id || "", password: "", active: true });
  ...
  async function create(event: React.FormEvent) {
    ...
    setForm(current => ({ ...current, email: "", name: "", password: "" }));
    router.refresh();
  }
  ...
  return <div className="admin-user-layout">
    <form className="panel admin-user-form" onSubmit={create}>
      ...
      <label className="field"><span>Temporary password</span><input type="password" minLength={12} maxLength={128} value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} required/></label>
